...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (i = 0; i < MAX_PROBES; i++) {
		 ...
		 */
		status = gen_iio_detect_probe(gen_iio_get_enrg_addr(i),
					      PROBE_NUM(i), ENRG_PROBE_NAME);
		if (status) {
			/* Energy probe detected. */
			status = gen_iio_register_probe(sdi, PROBE_ENRG,
					gen_iio_get_enrg_addr(i), PROBE_NUM(i));
			if (!status) {
			...
			status = gen_iio_detect_probe(gen_iio_get_temp_addr(i),
					      PROBE_NUM(i), TEMP_PROBE_NAME);
			if (status) {
				/* Temperature probe detected. */
				status = gen_iio_register_probe(sdi,PROBE_TEMP,
					gen_iio_get_temp_addr(i), PROBE_NUM(i));
				if (!status) {
		      ...
		      const struct sr_channel_group *cg)
{
	...
	case SR_CONF_PROBE_FACTOR:
		...
		ret = gen_iio_get_shunt(cg, &shunt);
		if (ret == SR_OK)
	...
	case SR_CONF_POWER_OFF:
		if (!cg)
			return SR_ERR_CHANNEL_GROUP;
		ret = gen_iio_read_power_state(cg, &power_off);
		if (ret == SR_OK)
		      ...
		      const struct sr_channel_group *cg)
{
	...
	case SR_CONF_SAMPLERATE:
		...
		gen_iio_maybe_set_update_interval(sdi, samplerate);
		break;
	case SR_CONF_PROBE_FACTOR:
		if (!cg)
			return SR_ERR_CHANNEL_GROUP;
		ret = gen_iio_set_shunt(cg, g_variant_get_uint64(data));
		break;
	case SR_CONF_POWER_OFF:
		if (!cg)
			return SR_ERR_CHANNEL_GROUP;
		ret = gen_iio_set_power_off(cg, g_variant_get_boolean(data));
		break;
	default:
		ret = SR_ERR_NA;
	}
		       ...
		       const struct sr_channel_group *cg)
{
	...
	if (!cg) {
		...
		switch (key) {
		case SR_CONF_DEVICE_OPTIONS:
			if (gen_iio_get_probe_type(cg) == PROBE_ENRG)
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi, void *cb_data)
{
	...
	sr_session_source_add_channel(sdi->session, devc->channel,
		G_IO_IN | G_IO_ERR, 1000, gen_iio_receive_data, (void *)sdi);
