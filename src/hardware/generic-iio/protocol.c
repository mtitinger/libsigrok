...
SR_PRIV uint8_t gen_iio_get_enrg_addr(int index)
{
	return enrg_i2c_addrs[index];
}

SR_PRIV uint8_t gen_iio_get_temp_addr(int index)
{
	return temp_i2c_addrs[index];
}

SR_PRIV gboolean gen_iio_is_sane(void)
{
	gboolean status;
...
SR_PRIV gboolean gen_iio_detect_probe(unsigned int addr,
...
SR_PRIV int gen_iio_get_probe_type(const struct sr_channel_group *cg)
{
...
SR_PRIV int gen_iio_probe_has_pws(const struct sr_channel_group *cg)
{
	struct channel_group_priv *cgp = cg->priv;
 ...
 * to samplerate.
 */
SR_PRIV void gen_iio_maybe_set_update_interval(const struct sr_dev_inst *sdi,
					       uint64_t samplerate)
{
	struct sr_channel_group *cg;
	struct channel_group_priv *cgp;
...
SR_PRIV int gen_iio_get_shunt(const struct sr_channel_group *cg,
			      uint64_t *shunt)
{
	GString *path = g_string_sized_new(64);
	gchar *contents;
...
SR_PRIV int gen_iio_set_shunt(const struct sr_channel_group *cg, uint64_t shunt)
{
	GString *path = g_string_sized_new(64);;
	int status, ret = SR_OK;
	FILE *fd;
...
SR_PRIV int gen_iio_read_power_state(const struct sr_channel_group *cg,
				     gboolean *off)
{
	struct channel_group_priv *cgp;
	int val;
...
SR_PRIV int gen_iio_set_power_off(const struct sr_channel_group *cg,
				  gboolean off)
{
	struct channel_group_priv *cgp;
	int val;
...
SR_PRIV int gen_iio_open_channel(struct sr_channel *ch)
{
	struct channel_priv *chp;
	char path[64], *file;
	int fd;
...
SR_PRIV void gen_iio_close_channel(struct sr_channel *ch)
{
	struct channel_priv *chp;
